...
- **German (de)**: Unlock language enabled.
- **Danish (da)**: Unlock language enabled.
- **Dutch (nl)**: Unlock language enabled.
- **Polish (pl)**: Unlock language enabled.
- **Spanish (es)**: Unlock language enabled.
- **French (fr)**: Unlock language enabled.

## 1a. Language -> Culture Unlock Map

Enabled language means the validator stack can accept that language in culture content. It does not mean every culture using that language is already complete.

- **English (en)** -> Global fallback route (used for Tier 2 and baseline content).
- **German (de)** -> Germany, Austria, and the German-language part of Switzerland (multi-language country).
- **Danish (da)** -> Denmark.
- **Dutch (nl)** -> Netherlands and Dutch-language Belgium scope.
- **Polish (pl)** -> Poland.
- **Spanish (es)** -> Spain and large parts of the Americas.
- **French (fr)** -> France, French-language Belgium, French-language Switzerland, and wider Francophone regions.

Note: Multi-language countries must be modeled explicitly as shared-language coverage (for example, Switzerland), not as a single-language country.
...
2. **French (fr)**: Infrastructure enabled (language registry + detector). Next step is country-level bag creation.
