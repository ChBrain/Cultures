...
Schedules validation as a linear chain (L1a -> L1b -> L2 -> L4 -> L4h):

  L1a:  General file format (UTF-8 no BOM, ASCII filenames, no em-dash)
  L1b:  English-only language policy
  L2:   Section structure (canonical headers per *khai: <type>* declaration)
  L4:   Culture completeness (per-country minimums)
  L4h:  History arc (defining moments: Yearbook >= 12, span >= 5 centuries)

L3 (link integrity) remains CI-only via the khai_tests pytest jobs. The
khai_tests components also run the same L2 (sections) check in CI; this
script provides the LOCAL parity exposed by the PR #105 incident where
a history file with narrative essay sections (instead of the canonical
*khai: piece* Place / Load Bearing / Apparent / Yearbook) passed the
hook and only failed at CI.

Each layer blocks subsequent layers on failure. This script is the
single entry point for the pre-commit hook; CI jobs call validators
individually for visibility in GitHub Actions.
...
def run_layer(name: str, module, files: list[str]) -> tuple[int, list[Issue]]:
    ...
    try:
        ...
        # - validate_general, validate_language: validate(path: Path)
        # - validate_sections, validate_culture, validate_history_arc:
        #   validate(changed_files: list[Path] | None)

        if any(name in module.__name__ for name in (
            "links", "culture", "sections", "history_arc"
        )):
...
def main():
    ...
    # Run validation chain (L1a -> L1b -> L2 -> L4 -> L4h)
    # Note: L0 (stamp gate) only runs in CI (see .github/workflows/validate.yml)
    # Note: L3 (links) remains CI-only via khai_tests pytest jobs.
    # Note: L2 (sections) and L4h (history arc) run BOTH locally and in CI.
    #       Local parity exists because the PR #105 incident showed that
    #       CI-only enforcement lets malformed files reach release branches
    #       and create unmergeable conflicts.

    all_passed = True
    layers = [
        ("L1a - General format", validate_general),
        ("L1b - English-only language", validate_language),
        ("L2 - Section structure (per khai declaration)", validate_sections),
        ("L4 - Culture completeness", validate_culture),
        ("L4h - History arc (defining moments)", validate_history_arc),
    ]
